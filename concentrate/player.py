...
class player0:
    ...
    def changedifficulty(self, diff):
        ...
        self.cache = dict() #cache saves the word list for the board, which changes if the dictionary changes
    ...
    def possible(self, letters):
        ...
        if letters in self.cache:
            return [x for x in self.cache[letters][0] if x not in self.cache[letters][1]] #avoid suggesting words that have already been played
        else:
            found = list()
    ...
    def concentrate(self, allletters, needletters='', notletters='', anyletters=''):
        ...
        if notletters != '': #remove words that use the notletters
            for word in allletterlist:
                good = True
                for l in notletters:
                    if word.count(l) > max(allletters.count(l) - notletters.count(l),0): #using 1 R when there are two Rs is fine
                        good = False
                        break
                if good:
    ...
    def endgamecheck(self, allletters, blue, red, move):
        '''checks to see if the opponent can end the game and win on the next 
        turn.  runs much faster than full 2 ply depth'''
        zeroletters = ''
        zeros = (~blue & ~red)
        anyl = ''
        ...
        endingsoon = False
        newscore = None
        if zeroletters != '':  #don't check if the game has finished
    ...
    def ply2(self, allletters, blue, red, move):
        '''returns the value of the position after the opponent's best move'''
    ...
    def playword(self, allletters, word):
        '''add word to list of words played on this board'''
        self.cache[allletters][1].append(word)

    def resetplayed(self, allletters, words):
        '''make the list played on this board exactly words'''
        self.cache[allletters][1] = words

    def unplayword(self, allletters, word):
        '''remove word from list of words played on this board'''
    ...
    def turn(self, allletters, score='wwwwwwwwwwwwwwwwwwwwwwwww', move=1):
        '''Selects a result from decide.  Used for playing against Concentrate'''
        allletters = allletters.upper()
        score = score.upper()
        score = score.replace(' ','')
    ...
    def computegoal(self, allletters, blue, red, move):
        '''Return the favorite smallest group of letters that have no playable 
        spanning words.  I want to leave the opponent with these letters so if
        he uses a single letter from that set, I can finish the game by using
        the rest.  If multiple small goals are found, pick the one farthest
        from my territory.'''
        goodgoals = []
    ...
    def goalvalue(self, goal, blue, red, move):
        '''better goals are those that are farther away from my squares'''
        val = 0
        if move == 1:
    ...
    def centroid(self, map):  # had an idea to look up this value from a reference file, but that file would be almost 1 GB in size using pickle.
        '''returns the coordinates of the center of mass of the 5x5 board.
        For strategical purposes it is an advantage to have a center of mass 
        farther away from the unoccupied squares than the opponent's center 
        of mass.  So this function is generally called on three maps, my
        squares, my opponent's squares, and the unoccupied squares, so I can 
        tell who has the better position.
        '''
        cnt = 0
        ysum = 0
        xsum = 0
...
##            return (2,2)
    ...
    def defended_map(self,posmap):
        ...
        This is a strict subset, including only squares that are completely 
        surrounded.'''
        n = self.neighbors
        defmap = 0
        for i in range(25):
    ...
    def playissafe(self, group, play):
        '''
        Is it safe to choose play in a group of words that have the same score?
        The goal is to choose the word in the group so that I end up with the 
        last word in that group.  Consider the following group of words that 
        have the same score when played:
            REPLAY
              REPLAYS (suffix of REPLAY)
              REPLAYED (suffix of REPLAY)
        This one double (REPLAY with its suffixes). The only safe choice here 
        is REPLAY.  This leaves the opponent with two singles.  If the opponent
        chooses one of these, I play the other to play the last word in the 
        group.  If I had chosen REPLAYS from the beginning, this means REPLAY
        is not available any longer, and REPLAYED is the last word in the 
        group, and the opponent can play it.  So REPLAYS is not safe.
        '''
        ...
        for i,word1 in enumerate(group):
            ...
            if word1 not in children:
                ...
                if mychildren:
                    ...
                    category['single'] += 1
        if category['big']:
            return False  #assume all plays the leave a tree of 3 or bigger are unsafe
        else:
...
class player1(player0):
    '''In order to test whether a change to the algorithm improves play, make
    the change here, overriding the parent player0 methods.  Then use arena.py
    to have the different versions play against each other.'''
    def __init__(self, difficulty=['A',5,25,'S'], weights = (3.1, 1.28, 2.29, 7.78)): #this represents maximum difficulty
